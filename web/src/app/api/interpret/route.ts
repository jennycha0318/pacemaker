...
export async function POST(req: Request) {
  ...
  const userText = [
    ...
    ``,
    `[작업]`,
    `1) interpretation: 먼저 첫 문장에서 사용자가 지금 느낄 감정을 구체적으로 명명하고 "그럴 만하다"고 정상화하세요(사용자가 freeText에 직접 쓴 표현을 인용; 막연한 일반론 위로 금지). 그다음 문장부터 위 결과와 일관되게, 사용자가 적은 내용과 '상대에 대한 설명'을 반영해 따뜻하고 정직하게 해석하세요(총 3~5문장). 상대 설명이 있으면 상대의 애착 성향(안정/불안/회피)·관심도·주의 신호를 조심스럽게 추론해 녹이되 단정하지 마세요.`,
    `2) message: ${
      wantMessage
