...
export async function POST(req: Request) {
  ...
  const userText = [
    ...
    prevInsight ? `[직전 진단 회고 — 이번 진단에 참고]\n${prevInsight}\n→ 사용자가 '잘됨/맞음'이라 한 접근은 이어가되('지난번에 ~한 게 잘 통했으니 이번엔…') 그대로 반복하지 말고 진전된 다음 스텝으로, '아쉬움/틀림'이면 접근을 바꾸세요. 통찰은 직전과 다른 각도로. 회고를 반영해 사용자가 '나를 기억한다'고 느끼게 하세요. 상황이 좋아졌으면(잘됨) 같은 '기다려라·자제하라' 조언을 글자 그대로 반복하지 말고, 나아진 점을 인정한 뒤 규칙 결과 범위 안에서 한 단계 진전된 코칭(다음 행동·다음 단계 준비)으로 전개하세요.` : "",
    ...
    `5) prediction: 향후 1~2주 내 사용자가 직접 맞았는지 확인할 수 있는 반증 가능한 예측 1개(1~2문장). 규칙 결과·추천 행동과 일관되게, 관찰 가능한 구체 사건으로(예: 추천대로 했을 때 상대의 예상 반응). 마땅하지 않으면 빈 문자열. 직전 회고가 주어졌으면 '직전 예측'을 그대로 반복(복붙)하지 마세요 — 지난 예측이 빗나갔으면(반반/틀림) 다른 각도의 예측을, 맞았으면(맞음) 한 단계 진전된 새 예측을 내세요.`,
  ]
    .filter(Boolean)
    .join("\n");
