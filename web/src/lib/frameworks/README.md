# lib/frameworks — 심리 프레임워크 규칙 레이어 ★ (계획 / 미구현)

> ⚠️ **현재 미구현(빈 디렉터리).** 이 폴더는 향후 차별점이 될 규칙 레이어의 설계 메모다.
> 현재 MVP의 진단은 `web/src/lib/diagnose/engine.ts`의 단일 규칙 엔진(결정적)으로 산출되며, 아래 모듈/LLM 주입은 아직 없다.

**계획상 이 폴더가 큐핏의 핵심 차별점이 된다.** (PRD §11-2)
