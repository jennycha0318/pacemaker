...
## 8. 미완 · 예정
- **챗봇(진단 Q&A):** ✅ 구현(무료) — `/chat` 페이지 + `/api/chat`(**Sonnet 4.6**), 하단 탭 '채팅'(히스토리 옆). 로그인 시 최근 진단을 컨텍스트로, 닉네임 호칭, 반말 금지. 최근 12턴 제한. **결제(메시지당 과금)는 미구현 = 다음 레이어.** 결제는 결정사항(결제사·가격·무료 한도) + 사용자의 결제계정/키 세팅 필요. ⚠️ 결제/금융 트랜잭션·크리덴셜 입력은 어시스턴트가 대신 못 함.
